...
description: Mapping the Enterprise Cyber Defense Doctrine to NIST Cybersecurity Framework (CSF) 2.0 Functions, Categories, and key Subcategories.
---

# Appendix — NIST CSF 2.0 Crosswalk

This appendix maps the **Enterprise Cyber Defense Doctrine** to the **NIST Cybersecurity Framework (CSF) 2.0**.  
Use it as a reference when building Profiles (Current/Target), setting Tiers, and prioritizing work.

> **Scope:** CSF 2.0 Functions (GV, ID, PR, DE, RS, RC), Categories (e.g., GV.RM), and representative Subcategories where helpful.  
> **Notes:** The Doctrine is outcome-oriented and business-aligned; CSF is outcome taxonomy. Multiple-to-multiple mappings are expected.

---

## High-level Crosswalk

| Doctrine Pillar / Chapter | Primary CSF Function(s) | CSF Category (IDs) | Notes / Exemplars |
|---|---|---|---|
| **Chapter 2 – People (Workforce & Career Pathing)** | GV, PR | **GV.RR** (roles, resourcing, HR), **GV.RM** (risk appetite informs staffing), **PR.AT** (awareness & training) | Map job architecture and career pathways to GV.RR-01..04; use PR.AT-01/02 for baseline and role-specific training. Tie staffing & skilling to risk appetite (GV.RM-02/03). |
| **Chapter 3 – Processes (Foundational Outcomes)** | GV, ID, PR, DE, RS, RC | **Govern:** GV.OC, GV.RM, GV.PO, GV.OV, GV.SC  • **Identify:** ID.AM, ID.RA, ID.IM • **Protect:** PR.AA, PR.AT, PR.DS, PR.PS, PR.IR • **Detect:** DE.CM, DE.AE • **Respond:** RS.MA, RS.AN, RS.CO, RS.MI • **Recover:** RC.RP, RC.CO | Use this to build Current/Target Profiles. Example: asset/business mapping → ID.AM-05/08; continuous monitoring → DE.CM-01/03/09; incident management & comms → RS.*; restoration & comms → RC.* |
| **Chapter 4 – Capability (Business-Aligned Outcomes)** | GV, ID, PR, DE, RS, RC | **GV.OC** (mission, stakeholders), **GV.RM** (strategy, risk options), **GV.OV** (performance & adjustments), **ID.IM** (improvement), **PR.IR** (resilience), **PR.PS-06** (secure SDLC) | Express capabilities in business terms (GV.OC-01/04). Drive prioritization via risk appetite (GV.RM-02/04). Bake resilience into architecture (PR.IR-01..04) and product lifecycle (PR.PS-06). |
| **Chapter 5 – Governance & Risk Alignment** | GV | **GV.OC, GV.RM, GV.RR, GV.PO, GV.OV, GV.SC** | Board/executive oversight (GV.OV-01..03), explicit risk appetite (GV.RM-02), ERM integration (GV.RM-03), C-SCRM end-to-end (GV.SC-01..10). |
| **Chapter 6 – Operationalizing the Doctrine** | GV, ID | **Profiles & Tiers (CSF §3)**, **ID.IM-01..04**, **GV.OV-01..03** | Build **Current vs. Target Profiles**, run **gap analysis** and POA&M, and set **Tier targets** per business risk. Institutionalize improvement loops via ID.IM and oversight via GV.OV. |
| **Chapter 7 – Continuous Renewal & Benchmarking** | GV, ID, PR, DE, RS, RC | **ID.IM-01..03** (learn & improve), **GV.OV-01..03** (review & adjust), **DE.AE-07** (integrate CTI), **PR.PS-04/06** (logging/SSDF), **PR.IR-03** (resilience), **RS.AN-06/07**, **RC.RP-04/05** | Red teaming & CTEM feed ID.IM and GV.OV. Capture evidence/forensics (RS.AN), verify restoration integrity (RC.RP-03/05), and roll lessons back into policy/architecture. |
| **Field Cards (Quick-Reference)** | (varies) | Map each card to the most relevant Categories | See mini-mappings below. |

---

## Mini-mappings for Field Cards

### Workforce Development (People)
- **GV.RR-01..04** — Accountability, role clarity, resourcing, HR integration  
- **PR.AT-01/02** — Awareness baseline + role training  
- **GV.RM-02/03** — Risk appetite & ERM drive staffing and skills

### Core Processes
- **Govern** → GV.OC, GV.RM, GV.PO, GV.OV, GV.SC  
- **Identify** → ID.AM-01..08, ID.RA-01..10, ID.IM-01..04  
- **Protect** → PR.AA-01..06, PR.AT-01/02, PR.DS-01/02/10/11, PR.PS-01..06, PR.IR-01..04  
- **Detect** → DE.CM-01/03/06/09, DE.AE-02/03/04/06/07/08  
- **Respond** → RS.MA-01..05, RS.AN-03/06/07/08, RS.CO-02/03, RS.MI-01/02  
- **Recover** → RC.RP-01..06, RC.CO-03/04

### Capability Doctrine
- **GV.OC-01/02/04/05** — Mission, stakeholders, dependencies  
- **GV.RM-02/04/06/07** — Appetite, risk options, quant/qual method, positive risk  
- **PR.IR-01..04** — Architectural resilience (availability, capacity, contingency)  
- **PR.PS-06** — Secure SDLC across lifecycle  
- **ID.IM-01..04** — Improvement engine

### Governance
- **GV.RM, GV.RR, GV.PO, GV.OV, GV.SC** — Strategy → Roles → Policy → Oversight → C-SCRM

### Operationalization
- **Profiles & Tiers** — Use CSF §3 to define scope, gather inputs, create Current/Target, gap, action plan, iterate  
- **ID.IM & GV.OV** — Use lessons and performance to recalibrate Profiles and risk strategy

### Continuous Renewal
- **ID.IM-01..03** — Learn from evaluations, tests, operations  
- **DE.AE-07** — Integrate threat intel into analysis  
- **RS/RC** — Preserve evidence, estimate magnitude, verify restore; update playbooks

---

## How to Use the Crosswalk

1. **Scope a Profile** (product line, business service, or threat scenario).  
2. **Bind Doctrine chapter goals** to the CSF Categories above.  
3. **Select Subcategories** as measurable outcomes; add your **Informative References** (e.g., 800-53, ISO 27001).  
4. **Set Tier targets** for GOVERN + the other Functions.  
5. **Run gap analysis → POA&M → Measure → Improve (ID.IM)**.

> **Source reference:** NIST CSF 2.0 (Feb 26, 2024) Functions, Categories, Subcategories; Profiles & Tiers.
